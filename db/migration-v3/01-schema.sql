-- =====================================================================
-- ONGLET 1 — Création du nouveau schéma
--
-- PRÉREQUIS : les deux lignes ci-dessous doivent avoir été exécutées
-- avant (elles mettent l'ancien schéma de côté sans rien détruire) :
--
--   ALTER SCHEMA public RENAME TO legacy;
--   CREATE SCHEMA public;
--
-- Le bloc RLS a été RETIRÉ de ce fichier : il est dans
-- rls-a-activer-plus-tard.sql, à n'exécuter qu'une fois que
-- app/core/db.py pose SET LOCAL app.project_id.
-- =====================================================================
...
CREATE TABLE core.publishing_targets (
  ...
  ga4_property_id             text,  -- GA4 propre à ce projet, repli sur settings.GA4_PROPERTY_ID si NULL
  ga4_service_account_json    text,  -- chiffré (Fernet), repli sur settings.GOOGLE_SERVICE_ACCOUNT_JSON si NULL
